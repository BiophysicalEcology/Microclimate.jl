...
function allocate_profile(heights, reference_height)
    ...
    N_heights = length(heights) + 1
    height_array = Vector{typeof(reference_height)}(undef, N_heights)
    height_array[1] = reference_height
    height_array[end:-1:2] .= u"m".(heights)
    wind_speeds = zeros(typeof(1.0u"m/minute"), N_heights) # output wind speeds
    air_temperatures = zeros(typeof(0.0u"K"), N_heights) # output temperatures, need to do this otherwise get InexactError
    humidities = zeros(Float64, N_heights) # output relative humidities
    # heights_orig = copy(heights)
    # heights = fill(0.0u"m", N_heights + 1)
    # heights[end:-1:2] .= height_array

    return (; heights, reference_height, height_array, wind_speeds, air_temperatures, humidities)
end
...
function get_profile!(buffers;
    ...
    (; heights, reference_height, height_array, wind_speeds, air_temperatures, humidities) = buffers

    minimum(heights) < z0 && _minimum_height_error(heights, z0)

    N_heights = length(air_temperatures)
    ...
    dry_air_out = dry_air_properties(u"K"(reference_temperature); elevation)
    ...
    if zh > 0.0u"m"
        # TODO ustrip to what
        sublayer_stanton_number = 0.62 / (ustrip(z0) * ustrip(u_star) / 12)^(9//20)
        ...
        if T_ref_height ≥ T_surface || T_surface ≤ u"K"(maximum_surface_temperature) || zenith_angle ≥ 90°
            # TODO ustrip to what
            sublayer_stanton_number = 0.62 / (ustrip(z0) * ustrip(u_star) / 12.0)^(9//20)
            bulk_stanton_number = 0.64 / dum
            Q_convection = ρ_cp * ΔT * u_star * bulk_stanton_number / (1.0 + bulk_stanton_number / sublayer_stanton_number)
