...
roughness length). A canopy model holds one of these in its `wind_model`
field.
...
Pre-allocate and precompute the structural (once-per-run) buffers an
[`AbstractCanopyWindModel`](@ref) needs.
"""
