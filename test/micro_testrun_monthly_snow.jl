...
environment_minmax = MonthlyMinMaxEnvironment(; forcings = minmax_forcings(;
    ...
    reference_wind_speed_min = (DataFrame(CSV.File("$testdir/data/init_monthly_snow/WNMINN.csv"))[days2do, 2] * 1.0)u"m/s", # min wind speed (m/s)
    reference_wind_speed_max = (DataFrame(CSV.File("$testdir/data/init_monthly_snow/WNMAXX.csv"))[days2do, 2] * 1.0)u"m/s", # max wind speed (m/s)
    ...
    cloud_cover_min = (DataFrame(CSV.File("$testdir/data/init_monthly_snow/CCMINN.csv"))[days2do, 2] * 1.0) ./ 100.0, # min cloud cover (fractional)
    cloud_cover_max = (DataFrame(CSV.File("$testdir/data/init_monthly_snow/CCMAXX.csv"))[days2do, 2] * 1.0) ./ 100.0, # max cloud cover (fractional)
))
