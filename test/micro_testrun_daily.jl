...
micro_out = runmicro(;
    ...
    days = days, # days of year to simulate - TODO leap years
    ...
    soil_mineral_conductivity = (CSV.File("test/data/init_daily/soilprop.csv")[1, 1][4]) * 1.0u"W/m/K", # soil minerals thermal conductivity (W/mC)
    soil_mineral_density = (CSV.File("test/data/init_daily/soilprop.csv")[1, 1][6]) * 1.0u"Mg/m^3", # soil minerals density (Mg/m3)
    soil_mineral_heat_capacity = (CSV.File("test/data/init_daily/soilprop.csv")[1, 1][5]) * 1.0u"J/kg/K", # soil minerals specific heat (J/kg-K)
    soil_bulk_density = (CSV.File("test/data/init_daily/soilprop.csv")[1, 1][2]) * 1.0u"Mg/m^3", # dry soil bulk density (Mg/m3)
    soil_saturation_moisture = (CSV.File("test/data/init_daily/soilprop.csv")[1, 1][3]) * 1.0u"m^3/m^3", # volumetric water content at saturation (0.1 bar matric potential) (m3/m3)
    # soil moisture model soil parameters
    air_entry_water_potential = (DataFrame(CSV.File("test/data/init_daily/PE.csv"))[:, 2] * 1.0u"J/kg"), # set up vector of ground emissivities for each day
    saturated_hydraulic_conductivity = (DataFrame(CSV.File("test/data/init_daily/KS.csv"))[:, 2] * 1.0u"kg*s/m^3"), # set up vector of ground emissivities for each day
    Campbells_b_parameter = (DataFrame(CSV.File("test/data/init_daily/BB.csv"))[:, 2] * 1.0), # set up vector of ground emissivities for each day
    soil_bulk_density2 = (DataFrame(CSV.File("test/data/init_daily/BD.csv"))[:, 2] * 1.0u"Mg/m^3"), # set up vector of ground emissivities for each day
    soil_mineral_density2 = (DataFrame(CSV.File("test/data/init_daily/DD.csv"))[:, 2] * 1.0u"Mg/m^3"), # set up vector of ground emissivities for each day
    # soil moisture plant parameters
    root_density = DataFrame(CSV.File("test/data/init_daily/L.csv"))[:, 2]u"m/m^3", # root density at each node, mm/m3 (from Campell 1985 Soil Physics with Basic, p. 131) # max depth for water pooling on the surface, mm (to account for runoff)
    ...
    albedos = (DataFrame(CSV.File("test/data/init_daily/REFLS.csv"))[:, 2] * 1.0), # substrate albedo (decimal %)
    shades = (DataFrame(CSV.File("test/data/init_daily/MINSHADES.csv"))[:, 2] * 1.0), # daily shade (%)
    pctwets = (DataFrame(CSV.File("test/data/init_daily/PCTWET.csv"))[:, 2] * 1.0),
    sles = (DataFrame(CSV.File("test/data/init_daily/SLES.csv"))[:, 2] * 1.0), # - surface emissivity
    daily_rainfall=((DataFrame(CSV.File("test/data/init_daily/rain.csv"))[:, 2] * 1.0))u"kg/m^2",
    air_temperature_min = (DataFrame(CSV.File("test/data/init_daily/TMINN.csv"))[:, 2] * 1.0)u"°C", # minimum air temperatures (°C)
    air_temperature_max = (DataFrame(CSV.File("test/data/init_daily/TMAXX.csv"))[:, 2] * 1.0)u"°C", # maximum air temperatures (°C)
    wind_min = nothing,
    wind_max = nothing,
    ...
    air_temperatures = Float64.(CSV.File("test/data/init_daily/TAIRhr.csv").x)u"°C",
    humidities = clamp.(Float64.(CSV.File("test/data/init_daily/RHhr.csv").x), 0, 100),
    wind_speeds = clamp.(Float64.(CSV.File("test/data/init_daily/WNhr.csv").x)u"m/s", 0.1u"m/s", (Inf)u"m/s"),
    solar_radiation = Float64.(CSV.File("test/data/init_daily/SOLRhr.csv").x)u"W/m^2",
    cloud_covers = clamp.(Float64.(CSV.File("test/data/init_daily/CLDhr.csv").x), 0, 100),
    RAINs = clamp.(Float64.(CSV.File("test/data/init_daily/RAINhr.csv").x)u"kg" / u"m^2", 0u"kg/m^2", (Inf)u"kg/m^2"),
    zenith_angles=nothing,
    longwave_radiation=nothing,
    # intial conditions
    initial_soil_temperature = u"K".((DataFrame(CSV.File("test/data/init_daily/soilinit.csv"))[1:length(depths), 2] * 1.0)u"°C"), # initial soil temperature
    initial_soil_moisture = (Array(DataFrame(CSV.File("test/data/init_daily/moists.csv"))[1, 2:13]) .* 1.0), # initial soil moisture
    leaf_area_index = (DataFrame(CSV.File("test/data/init_daily/LAI.csv"))[:, 2] * 1.0u"Mg/m^3"), # leaf area indices per day
