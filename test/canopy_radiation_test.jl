...
model = Microclimate.example_multilayer_canopy(;
    ...
    leaf_reflectance = 0.1, leaf_transmittance = 0.05,
)
heights = vcat(0.1:0.1:1.0, [2.0]) .* u"m"
buffers = Microclimate.allocate_canopy(model, heights)
n_layers = length(buffers.layer_plant_area_index_above)
...
@testset "diffuse-only irradiance" begin
    ...
    @test all(iszero, buffers.sunlit_fraction)
end

@testset "sunlit fraction decays monotonically with depth" begin
    Microclimate.canopy_radiation!(buffers, model;
    ...
    @test issorted(buffers.sunlit_fraction, rev=true)
    @test all(f -> 0.0 <= f <= 1.0, buffers.sunlit_fraction)
end
...
@testset "night returns zero absorption" begin
    ...
    @test all(iszero, buffers.absorbed_shortwave)
end

@testset "NoCanopy allocates nothing" begin
    @test Microclimate.allocate_canopy(NoCanopy(), heights) === nothing
end
